...
ファイル名は「8.3 形式」のままですが、漢字コードは JIS コードに変換して
あります。
...
1. このパッケージに含まれているファイルを TeX ライブラリディレクトリで
   展開をします。展開すると pl209 ディレクトリが作成されます。

   例：cd /usr/local/share/texmf/tex
       gunzip -cd SOMEWHERE/pl209.tar.gz | tar xvf -
...
5. /usr/local/share/texmf/web2c/texmf.cnf ファイルを編集します。
   TEXINPUTS.jlplain および TEXINPUTS.plplain の設定に pl209
   ディレクトリが含まれるようにしてください。

   例：platex209_inputs=.:$dbtex/{pl209,latex209,platex,latex,generic}//
...
このパッケージで配布されている「8.3 形式」の名前のファイルを本来のファイル名
で用いることができます。左側がディスク上のファイル名、右側が別名として用いる
名前です。

am_webma.tex    am_webmac.tex
ascjlett.sty    ascjletter.sty
asclette.sty    ascletter.sty
jtwocolu.sty    jtwocolumn.sty
landscap.sty    landscape.sty
latexpic.tex    latexpicobjs.tex
postpict.tex    postpictex.tex
prepicte.tex    prepictex.tex
tgrindma.tex    tgrindmac.tex
titlepag.doc    titlepage.doc
titlepag.sty    titlepage.sty
twocolum.doc    twocolumn.doc
twocolum.sty    twocolumn.sty
