...
1). はじめに
  ...
  TeX 処理系の WIN32 DLL 版です。Web2c 7.0 及び ASCII pTeX 2.1.6 をベー
スにしています。
...
5). アンインストール
  ...
  アンインストールには付属のアンインストールプログラムを使います。
  セットアッププログラムを使ってインストールした場合には
  「コントロールパネル」の「プログラムの追加と削除」に
  「TeX DLL」という名前で登録されていますので、それを選んでください。
  セットアッププログラムを使わずにインストールした場合には
  インストール先ディレクトリの「setup\uninst.exe」を起動してください。

  アンインストールはユーザ毎の設定を消去するためにユーザアカウント毎に
  おこなう必要が有ります。NT ではシステム管理者以外は、レジストリ中の
  ユーザ設定の消去のみがおこなえます。システム管理者は、インストールされた
  ファイルの削除、およびセットアップ情報の消去もさらにおこなえます。

  アンインストールプログラムは、自分自身を削除せずに残します。
  これは、ユーザ毎の設定を全員が消去し終えるまでは必要だからです。
  不要になったら手動で削除してください。

  また、環境変数 PATH に関しては手をつけずにそのまま残します。
  不要なもの (TeX の *.exe があるディレクトリ) は各自削除してください。
...
9). 最新パッケージ及び情報の入手方法
  ...
  このパッケージの一次頒布元の URL は以下のとおりです。
    ...
    http://ascii.asciimw.jp/pb/ptex/archives/ascii-ptex/ptex215/windll/tex/
